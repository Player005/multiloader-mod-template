    ...
    withType<ProcessResources>().matching(notNeoTask).configureEach {
        ...
        filesMatching("data/**/*.json") {
            if (inputs.properties["handle_fluid_conversion"].toString().toBoolean())
                convertFluidUnitsForge()
            if (inputs.properties["unified_load_conditions"].toString().toBoolean())
                processUnifiedLoadConditionsForge()
        }
    }
}


fun ContentFilterable.convertFluidUnitsForge() {
    filter { line ->
...
fun ContentFilterable.processUnifiedLoadConditionsForge() {
    filter { line ->
